...
function agregarPoliza(id, nombreCliente, montoAsegurado, fechaRenovacion) {
    const nuevaPoliza = [id, nombreCliente, montoAsegurado, fechaRenovacion];
    polizas.push(nuevaPoliza);
    console.log(`Poliza añadida para ${nombreCliente}`);
}
// Funcion para renovación de poliza
function renovarPoliza(indice) {
    setTimeout(() => {
        if (polizas[indice]) {
